export const LogicSearch = dataGraphics => {
  ...
  dataGraphics.map(obj => {
    ...
    let element = true
    data.map((elem, i) => {
      if (elem.area_search == obj.area_search) {
        state = i
      }
    })
    ...
    if (!data[element - 1].state_search.Cerrada && element)
      data[element - 1].state_search.Cerrada = 0
    if (!data[element - 1].state_search.Nueva && element)
      data[element - 1].state_search.Nueva = 0
    if (!data[element - 1].state_search.Iniciada && element)
      data[element - 1].state_search.Iniciada = 0
  })
  return data
}
...
export const LogicRecruiter = dataGraphics => {
  ...
  arr.map(i => {
    dataGraphics.map(obj => {
      if (obj.active_searchs == i) data[i] = obj
    })
